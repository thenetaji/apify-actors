...
try {
  ...
  case "all":
      ...
      $(selector).each((_, el) => {
        ...
        if (src) {
          try {
            ...
            log.warning("Failed to process media URL", {
              src,
              error: e.message,
            });
          }
    ...
    if (crawledUrlsCount >= maxUrlsToCrawl) {
      log.info(
        `Reached maximum URL limit of ${maxUrlsToCrawl}. Stopping further URL enqueuing.`,
      );
      return;
    }
    ...
    if (depth < maxCrawlDepth) {
      ...
      const remainingCapacity = Math.max(
        0,
        remainingUrlsQuota - pendingRequestCount,
      );

      if (remainingCapacity <= 0) {
        log.info(
          "URL quota filled by pending requests. Not enqueueing more URLs.",
        );
        return;
      }
      ...
      log.debug(
        `Processing ${urlsToProcess.length} URLs out of ${links.length} found (quota: ${remainingCapacity})`,
        {
