...
export async function downloadYoutubeMusic(
  ...
  log.debug("Download parameters", { URL, selectedQuality, proxyURL, convertToMp3 });
  ...
  if (!quality) {
    const errorMessage = "Format or quality not matched with the provided values";
    log.error(errorMessage);
    throw new Error(errorMessage);
  }
  log.debug(`Selected audio quality: "${selectedQuality}" (mapped to "${quality}")`);
